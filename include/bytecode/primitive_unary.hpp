...
struct Bytecode_Primitive_Unary {
	template<typename T>
	static void logical_negate (void* dest, void* ptr) {
		T value;
	...
	static void arith_negate (void* dest, void* ptr) {
		T value;
		memcpy(&value, ptr, sizeof(T));
		R result = value;
		result = -result;
	...
	template<typename T>
	static void bitwise_negate (void* dest, void* ptr) {
		T value;
		memcpy(&value, ptr, sizeof(T));
		value = ~value;
		memcpy(dest, &value, sizeof(T));
	...
	static void unary (uint8_t unop, void* dest, void* ptr, uint8_t bytecode_type) {
		switch (unop) {
