...
struct Bytecode_Primitive_Binary {
	template<typename T>
	static void binary_result (uint8_t binop, void* dest, void* a_ptr, void* b_ptr) {
		T a, b;
		int8_t r = -1;
	...
	template<typename T>
	static void binary_result_logic (uint8_t binop, void* dest, void* a_ptr, void* b_ptr) {
		T a, b;
		memcpy(&a, a_ptr, sizeof(T));
		memcpy(&b, b_ptr, sizeof(T));
		switch (binop) {
	...
	static void binary (uint8_t binop, void* dest, void* a_ptr, void* b_ptr, uint8_t bytecode_type) {
