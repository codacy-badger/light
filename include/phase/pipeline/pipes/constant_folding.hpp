...
Ast_Type* unary_type (Ast_Unary_Type unary_op, Ast_Expression* exp) {
	switch (unary_op) {
		case AST_UNARY_NEGATE: {
			...
			else if (exp->inferred_type == Types::type_s16) return Types::type_u8;
			else if (exp->inferred_type == Types::type_s32) return Types::type_u16;
			else if (exp->inferred_type == Types::type_s64) return Types::type_u32;
			else if (exp->inferred_type == Types::type_u8) 	return Types::type_s16;
		...
		case AST_UNARY_NOT: return Types::type_bool;
		default: abort();
	}
}
		...
		if (binary->binary_op == AST_BINARY_ATTRIBUTE) {
			...
			if (binary->lhs->inferred_type->typedef_type == AST_TYPEDEF_ARRAY) {
				...
				if (strcmp(ident->name, "length") == 0) {
					...
					array_ref->inferred_type = new Ast_Pointer_Type(arr_type->base);
